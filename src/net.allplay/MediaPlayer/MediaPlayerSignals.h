...
public interface class IMediaPlayerSignals
{
	...
	event Windows::Foundation::TypedEventHandler<MediaPlayerSignals^, MediaPlayerPlayStateChangedReceivedEventArgs^>^ PlayStateChangedReceived;
};
...
public:
	...
	// Calling this method will send the ShuffleModeChanged signal to every member of the session.
	void PlayStateChanged(_In_ net::allplay::MediaPlayer::MediaPlayerPlayState^ playState);

	// This event fires whenever the PlayStateChanged signal is sent by another member of the session.
	virtual event Windows::Foundation::TypedEventHandler<MediaPlayerSignals^, MediaPlayerPlayStateChangedReceivedEventArgs^>^ PlayStateChangedReceived
	{
		Windows::Foundation::EventRegistrationToken add(Windows::Foundation::TypedEventHandler<MediaPlayerSignals^, MediaPlayerPlayStateChangedReceivedEventArgs^>^ handler)
		{
			return _PlayStateChangedReceived += ref new Windows::Foundation::EventHandler<Platform::Object^>
				([handler](Platform::Object^ sender, Platform::Object^ args)
			{
				handler->Invoke(safe_cast<MediaPlayerSignals^>(sender), safe_cast<MediaPlayerPlayStateChangedReceivedEventArgs^>(args));
			}, Platform::CallbackContext::Same);
		}
		void remove(Windows::Foundation::EventRegistrationToken token)
		{
			_PlayStateChangedReceived -= token;
		}
	internal:
		void raise(MediaPlayerSignals^ sender, MediaPlayerPlayStateChangedReceivedEventArgs^ args)
		{
			_PlayStateChangedReceived(sender, args);
		}
	}
internal:
	...
	void CallPlayStateChangedReceived(_In_ MediaPlayerSignals^ sender, _In_ MediaPlayerPlayStateChangedReceivedEventArgs^ args);
...
private:
	...
	virtual event Windows::Foundation::EventHandler<Platform::Object^>^ _PlayStateChangedReceived;
	...
	alljoyn_interfacedescription_member m_memberPlayStateChanged;
};
