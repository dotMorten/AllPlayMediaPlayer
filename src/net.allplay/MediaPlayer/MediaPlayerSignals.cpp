...
void MediaPlayerSignals::Initialize(_In_ alljoyn_busobject busObject, _In_ alljoyn_sessionid sessionId)
{
	...
	alljoyn_interfacedescription_getmember(interfaceDefinition, "PlayStateChanged", &m_memberPlayStateChanged);
...
void MediaPlayerSignals::CallPlayStateChangedReceived(_In_ MediaPlayerSignals^ sender, _In_ MediaPlayerPlayStateChangedReceivedEventArgs^ args)
{
	PlayStateChangedReceived(sender, args);
}

void MediaPlayerSignals::PlayStateChanged(_In_ net::allplay::MediaPlayer::MediaPlayerPlayState^ playState)
{
	if (nullptr == m_busObject)
	{
		return;
	...
	size_t argCount = 1;
	alljoyn_msgarg arguments = alljoyn_msgarg_array_create(argCount);
	(void)TypeConversionHelpers::SetAllJoynMessageArg(alljoyn_msgarg_array_element(arguments, 0), "(sxuuuiia(ssssxsssa{ss}a{sv}v))", playState);

	alljoyn_busobject_signal(
		m_busObject,
		NULL,  // Generated code only supports broadcast signals.
		m_sessionId,
		m_memberPlayStateChanged,
		arguments,
		argCount,
		0, // A signal with a TTL of 0 will be sent to every member of the session, regardless of how long it takes to deliver the message
		ALLJOYN_MESSAGE_FLAG_GLOBAL_BROADCAST, // Broadcast to everyone in the session.
