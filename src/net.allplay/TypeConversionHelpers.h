...
internal:
    ...
    static _Check_return_ int32 SetAllJoynMessageArg(_In_ alljoyn_msgarg argument, _In_ PCSTR signature, _In_ Platform::Object^ value)
    {
		...
		auto stringIntMapValue = dynamic_cast<Windows::Foundation::Collections::IMap<Platform::String^, int32>^>(value);
		if (stringIntMapValue != nullptr)
		{
			return SetVariantArg(argument, "a{si}", stringIntMapValue);
		}
		auto stringStringMapValue = dynamic_cast<Windows::Foundation::Collections::IMap<Platform::String^, Platform::String^>^>(value);
		if (stringStringMapValue != nullptr)
		{
			return SetVariantArg(argument, "a{ss}", stringStringMapValue);
		}
		if (value == nullptr)
		{
			alljoyn_msgarg variantArg = alljoyn_msgarg_create();
			//RETURN_IF_QSTATUS_ERROR(alljoyn_msgarg_set(variantArg, signature, nullptr));
			QStatus status = alljoyn_msgarg_set_and_stabilize(argument, "v", variantArg);
			alljoyn_msgarg_destroy(variantArg);
			return status;

			return ER_OK;
			//return SetVariantArg(argument, "v", nullptr);
		}
